...
- Mamie's notebook (G4) replaced the hint bubble: the top-right button opens it on the Notes page (every note read on this walk, newest first, grouped by room) with a Hints page next to it. A walk's notebook is saved (`notebook` in the save) and starts empty in the walk's first room; other modes keep notes for the one room only. Only paper notes (`takeable` props) leave their spot; pencil marks and photos stay in the room, but what they say is in the notebook too. Counting clues (a jar of shells) aren't copied, because the room object is the clue.
- The bag is see-through (55%) until it's hovered or an item is selected, shows only the bag button while it's empty (the "things you pick up" line is only in the tutorial), and can be folded away with the bag button. A folded bag peeks open for 4 seconds when something goes in. The folded state is a setting, so it stays folded in the next rooms.
